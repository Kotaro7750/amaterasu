...
int execHandler2(char *filename) {
  ...
  unsigned long long entryPoint = ((struct Elf64_Ehdr *)buffer)->e_entry;
  Elf64_Off e_phoff = ((struct Elf64_Ehdr *)buffer)->e_phoff;
  Elf64_Half e_phentsize = ((struct Elf64_Ehdr *)buffer)->e_phentsize;
  Elf64_Half e_phnum = ((struct Elf64_Ehdr *)buffer)->e_phnum;

  struct Elf64_Phdr *phdr = (struct Elf64_Phdr *)(buffer + e_phoff);

  unsigned long long textSegmentAddress = phdr[0].p_vaddr;
  // TODO memsizeとファイル中でのサイズが違う時
